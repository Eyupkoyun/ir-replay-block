#include "aes_cmac.h"
#include <string.h>

/* ========= AES-128 CORE (tiny-AES style, minimal) ========= */

#define AES_BLOCKLEN 16
#define Nb 4
#define Nk 4
#define Nr 10

static uint8_t RoundKey[176];

static const uint8_t sbox[256] = {
  
};

static uint8_t xtime(uint8_t x) {
  return (x << 1) ^ ((x >> 7) * 0x1b);
}

static void KeyExpansion(const uint8_t *Key) {
  memcpy(RoundKey, Key, 16);
  uint8_t tempa[4];
  uint8_t i = 4;
  while (i < 44) {
    memcpy(tempa, &RoundKey[(i - 1) * 4], 4);
    if (i % 4 == 0) {
      uint8_t t = tempa[0];
      tempa[0] = sbox[tempa[1]] ^ (1 << ((i / 4) - 1));
      tempa[1] = sbox[tempa[2]];
      tempa[2] = sbox[tempa[3]];
      tempa[3] = sbox[t];
    }
    for (uint8_t j = 0; j < 4; j++)
      RoundKey[i * 4 + j] = RoundKey[(i - 4) * 4 + j] ^ tempa[j];
    i++;
  }
}

static void Cipher(uint8_t *state) {
  for (uint8_t round = 0; round < Nr; round++) {
    for (uint8_t i = 0; i < 16; i++) state[i] = sbox[state[i]];
    for (uint8_t i = 0; i < 16; i++) state[i] ^= RoundKey[round * 16 + i];
  }
}

/* ========= CMAC ========= */

static void xor_128(uint8_t *a, const uint8_t *b) {
  for (uint8_t i = 0; i < 16; i++) a[i] ^= b[i];
}

void aes_cmac(const uint8_t *key, const uint8_t *msg, uint16_t len, uint8_t *mac_out) {
  uint8_t X[16] = {0};
  uint8_t Y[16];
  uint8_t M[16];

  KeyExpansion(key);

  while (len > 16) {
    memcpy(M, msg, 16);
    memcpy(Y, X, 16);
    xor_128(Y, M);
    Cipher(Y);
    memcpy(X, Y, 16);
    msg += 16;
    len -= 16;
  }

  memset(M, 0, 16);
  memcpy(M, msg, len);
  M[len] = 0x80;

  memcpy(Y, X, 16);
  xor_128(Y, M);
  Cipher(Y);

  memcpy(mac_out, Y, 16);
}
